...
// make the 1-Wire bus high by setting port bit for input
...
/*
  ds_convert - Send a "CONVERT_T" command to start a conversion (don't have to wait for finish)
*/
void ds_convert()
{
    if (ds_reset())
...
char ds_temp(unsigned char *tdata)
{
    ...
    if (ds_readbit())
    { // Wait for a read to return a one
        if (ds_reset())
        {                                     // Reset the DS18B20
            ds_writebyte(DS_SKIP_ROM);        // Send a "Skip ROM" command
            ...
            *tdata = ds_readbyte();           // Read the second byte (MSB)
            for (i = 0; i < 7; i++)
            { // Read the rest of the scratchpad data
                (void)ds_readbyte();
            }
        }
        return (1);
...
/*
  ...
  wait 67usec.
  Send a reset pulse and check for presence pulse after delay (anywhere from 15-75 usec to 60-120)
  -delay 67 to split difference and further delay to ensure full pulse
*/
char ds_reset(void)
{
...
void ds_write1bit(void)
{
    ...
    //delay <15 us
    _delay_us(2);
    DDRC &= ~(1 << PC3); //pull bus high
    _delay_us(60);       //delay to complete 62 us time slot
...
void ds_write0bit(void)
{
    ...
    _delay_us(60);
    DDRC &= ~(1 << PC3); //pull bus high
    _delay_us(2);        //delay to complete 62 us time slot
}
...
unsigned char ds_readbit(void)
{
    ...
    //pull bus low for 2 us, then release it (floating)
